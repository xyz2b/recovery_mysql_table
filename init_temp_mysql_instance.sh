...
mysql_gtid_set=`cat xtrabackup_binlog_info|cut -f 3|xargs`
mysql_start_gtid=`cat xtrabackup_binlog_info|cut -f 3|xargs|awk -F', ' '{i=1; while(i<=NF){if(match($i, /'$server_uuid'/) > 0) print $i; i++ }}'`
...
echo "mysql_gtid_set=$mysql_gtid_set"

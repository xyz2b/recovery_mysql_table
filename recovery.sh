...
mysql_server_uuid_gtid_no=`echo $mysql_gtid_set|awk -F', ' '{i=1; while(i<=NF){if(match($i, /'$server_uuid'/) > 0) print $i; i++ }}'|awk -F'-' '{print $NF}'`
mysql_gtid_set=`echo $mysql_gtid_set|awk -F', ' '{i=1; while(i<=NF){if(match($i, /'$server_uuid'/) > 0)  sub('$mysql_end_gtid_no', '$mysql_sql_brefore_gtid_no', $i); if(i==NF){ printf "%s",$i } else {printf "%s, ",$i}; i++ }}'`
